...
class GpuReader:
    ...
    def _init_nvml(self):
        try:
            ...
            if self.nvml.nvmlInit_v2() == 0:
                h = ctypes.c_void_p()
                if self.nvml.nvmlDeviceGetHandleByIndex_v2(0, byref(h)) == 0:
                    self.handle = h
                    self.available = True
                    return True
        except Exception:
    ...
    def read(self):
        ...
        try:
            ...
            r_temp = self.nvml.nvmlDeviceGetTemperature(self.handle, 0, byref(temp))
            r_pwr = self.nvml.nvmlDeviceGetPowerUsage(self.handle, byref(power))
            r_util = self.nvml.nvmlDeviceGetUtilizationRates(self.handle, byref(util))

            # 设备句柄失效或独显休眠重连 (NVML_ERROR_GPU_IS_LOST = 15)
            if r_temp == 15 or r_pwr == 15 or r_util == 15:
                self._init_nvml()
                return 0, 0.0, -1

            # 校验温度 (0~130°C)
            t_val = temp.value if (r_temp == 0 and 0 < temp.value < 130) else 0

            # 校验功耗 (0mW ~ 500000mW = 0W ~ 500W)，独显休眠或异常时为 0W，绝不传递 0xFFFFFFFF 溢出值
            if r_pwr == 0 and 0 <= power.value <= 500000:
                p_val = power.value / 1000.0
            else:
                p_val = 0.0

            # 校验占用率 (0~100%)
            u_val = util.gpu if (r_util == 0 and 0 <= util.gpu <= 100) else -1

            return t_val, p_val, u_val
        except Exception:
            return 0, 0.0, -1
...
def get_stats_json():
    ...
    payload = {
        "fps": int(fps) if (fps and 0 <= fps <= 999) else 0,
        "cpu": max(0, min(100, cpu_percent)),
        "mem": max(0, min(100, mem_percent)),
        "cpu_temp": int(round(cpu_temp)) if (cpu_temp and 0 < cpu_temp < 130) else 0,
        "cpu_pwr": int(round(cpu_pwr)) if (cpu_pwr and 0 <= cpu_pwr <= 350) else 0,
        "gpu_temp": int(round(gpu_temp)) if (gpu_temp and 0 < gpu_temp < 130) else 0,
        "gpu_pwr": int(round(gpu_pwr)) if (gpu_pwr and 0 <= gpu_pwr <= 450) else 0,
        "gpu_usage": int(round(gpu_usage)) if (gpu_usage is not None and 0 <= gpu_usage <= 100) else -1,
        "mode": mode if mode else "BALANCED",
...
async def run_ble_sender():
    ...
    fps_ok = fps_monitor.available
    fps_status = "已就绪 (G-Helper ETW)" if fps_ok else "启动失败"
    ...
    while True:
        ...
        try:
            async with BleakClient(device) as client:
                ...
                print(f"[成功] 蓝牙连接成功！(协商 MTU: {mtu} 字节)")
                if mtu < 150:
                    print(f"[提示] 当前协商 MTU 为 {mtu} 字节 (单包 JSON 需 ~140 字节)，若遇丢包请确认 ESP32 端已启用 setMTU(256)")
                print("[推流] 开始以 0.3s 极速推流硬件状态与游戏帧率...\n")
